...
def get_optimizers_and_schedulers(gen, disc):
    ...
    # The learning rate for the discriminator decaye to 0 over 500K steps.
    # The learning rate for the generator decaye to 0 over 100K steps.

    optim_discriminator = torch.optim.Adam(disc.parameters(), lr=2e-4, betas=(0, 0.9))
    lambda_gen = lambda epoch:  (5e5 - epoch) / 5e5
    scheduler_discriminator = torch.optim.lr_scheduler.LambdaLR(optim_discriminator,lr_lambda=lambda_gen)
    optim_generator = torch.optim.Adam(gen.parameters(), lr=2e-4, betas=(0, 0.9))
    lambda_disc = lambda epoch:  (1e5 - epoch) / 1e5
    scheduler_generator = torch.optim.lr_scheduler.LambdaLR(optim_generator,lr_lambda=lambda_disc)
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast():
                ...
                # compute generator outputs and discriminator outputs
                # print(train_batch.shape)
                fake_img = gen.forward(train_batch.shape[0])
                discrim_real = disc(train_batch)
                discrim_fake = disc(fake_img)

                # Compute the interpolated batch and run the discriminator on it.
                # draw eps ~ Uniform(0, 1)
                # interpolated data = eps * fake_data + (1-eps) * real_data
                eps = torch.rand(train_batch.shape[0], 1, 1, 1).cuda()
                interp = (eps * train_batch + ((1 - eps) * fake_img)).requires_grad_(True)
            ...
            if iters % 5 == 0:
                with torch.cuda.amp.autocast():
                    # Compute samples and evaluate under discriminator.
                    fake_img = gen.forward(train_batch.shape[0])
                    discrim_fake = disc(fake_img)
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.cuda.amp.autocast():
                        # Generate samples using the generator, make sure they lie in the range [0, 1].
